import { r as reactExports, c as useRetroSound, b as useGamification, j as jsxRuntimeExports, S as SquishyButton, L as Link } from "./index-DDxiqhuW.js";
import { G as GameOverCard } from "./GameOverCard-DHRVbdDZ.js";
import { feedService } from "./feed-C9DXiTgy.js";
const BIOMES = [
...
const MerchJump = () => {
  ...
  const generatePlatform = (y, biome) => {
    ...
    if (Math.abs(score2 - 4800) < 400 && Math.random() < 0.05 && itemsRef.current.length === 0) {
  ...
  const drawRig = (ctx2, x, y, vy, tilt) => {
    const time = performance.now() * 0.01;
    ctx2.save();
    ctx2.translate(x, y);
    ctx2.rotate(tilt * 0.2);
    ctx2.fillStyle = "#ccc";
    ctx2.fillRect(-15, -10, 10, 30);
    ctx2.fillRect(5, -10, 10, 30);
    if (vy < 0) {
      ctx2.fillStyle = "#ff9900";
      ctx2.beginPath();
      ctx2.moveTo(-10, 20);
      ctx2.lineTo(-5, 35 + Math.random() * 10);
      ctx2.lineTo(0, 20);
      ctx2.moveTo(10, 20);
      ctx2.lineTo(15, 35 + Math.random() * 10);
      ctx2.lineTo(5, 20);
      ctx2.fill();
    }
    ctx2.fillStyle = "#222";
    ...
    ctx2.beginPath();
    ctx2.moveTo(-10, 20);
    ctx2.quadraticCurveTo(-15, 25, -12, legLeftY);
    ctx2.lineWidth = 6;
    ctx2.strokeStyle = "#222";
    ctx2.stroke();
    ctx2.beginPath();
    ctx2.moveTo(10, 20);
    ctx2.quadraticCurveTo(15, 25, 12, legRightY);
    ctx2.stroke();
    ctx2.fillStyle = "white";
    ctx2.fillRect(-16, legLeftY, 8, 5);
    ctx2.fillRect(8, legRightY, 8, 5);
    ctx2.fillStyle = selectedSkin.hoodie || "#111";
    ctx2.beginPath();
    ctx2.roundRect(-20, 0, 40, 35, 10);
    ctx2.fill();
    ctx2.fillStyle = "white";
    ctx2.font = "10px sans-serif";
    ctx2.fillText("M", -5, 20);
    ctx2.strokeStyle = selectedSkin.hoodie || "#111";
    ctx2.lineWidth = 8;
    ctx2.lineCap = "round";
    ctx2.beginPath();
    ctx2.moveTo(-18, 5);
    ctx2.lineTo(-30, vy < 0 ? -10 : 15);
    ctx2.stroke();
    ctx2.beginPath();
    ctx2.moveTo(18, 5);
    ctx2.lineTo(30, vy < 0 ? -10 : 15);
    ...
    if (skinImgRef.current && skinImgRef.current.complete) {
      const size = 50;
      ctx2.drawImage(skinImgRef.current, -size / 2, -size / 2 - 15, size, size);
    } else {
      ctx2.fillStyle = "white";
      ctx2.beginPath();
      ctx2.arc(0, -20, 20, 0, Math.PI * 2);
      ctx2.fill();
    }
    ctx2.restore();
  };
  const gameLoop = () => {
    ...
    player2.x += (targetX - player2.x) * 0.15;
    if (player2.x < -20) player2.x = WIDTH + 20;
    if (player2.x > WIDTH + 20) player2.x = -20;
    player2.vy += GRAVITY;
    player2.y += player2.vy;
    if (player2.y < HEIGHT / 2) {
      const shift2 = HEIGHT / 2 - player2.y;
      player2.y = HEIGHT / 2;
      cameraYRef.current += shift2;
      scoreRef.current += Math.floor(shift2);
      setScore(scoreRef.current);
      platformsRef.current.forEach((p) => p.y += shift2);
      platformsRef.current = platformsRef.current.filter((p) => p.y < HEIGHT);
      itemsRef.current.forEach((i) => i.y += shift2);
    ...
    if (player2.vy > 0) {
      platformsRef.current.forEach((p, idx) => {
        if (player2.x > p.x - 20 && player2.x < p.x + p.w + 20 && player2.y + 30 > p.y && player2.y + 30 < p.y + p.h + 20) {
    ...
    itemsRef.current.forEach((item, idx) => {
      if (player2.x > item.x - 30 && player2.x < item.x + item.w + 30 && player2.y > item.y - 30 && player2.y < item.y + item.h + 30) {
    ...
    if (currentBiome.glitch && Math.random() > 0.95) {
      ctx2.save();
      ctx2.translate((Math.random() - 0.5) * 10, 0);
      if (Math.random() > 0.5) ctx2.filter = "invert(1)";
    }
    ...
    if (cloudBiomes.includes(currentBiome.name)) {
      ...
      if (currentBiome.name === "TOXIC WASTE") ctx2.fillStyle = "rgba(100, 255, 0, 0.2)";
      if (currentBiome.name === "ASCENSION") ctx2.fillStyle = "rgba(255, 215, 0, 0.2)";
      for (let i = 0; i < 5; i++) {
      ...
      ctx2.strokeStyle = "rgba(0, 255, 255, 0.2)";
      if (currentBiome.name === "VOLCANO") ctx2.strokeStyle = "rgba(255, 0, 0, 0.3)";
      if (currentBiome.name === "MIDNIGHT TOKYO") ctx2.strokeStyle = "rgba(255, 0, 255, 0.3)";
      ctx2.lineWidth = 2;
    ...
    platformsRef.current.forEach((p) => {
      ctx2.fillStyle = p.color || currentBiome.plat;
      if (p.type === "crumble") ctx2.fillStyle = "#A0522D";
      ctx2.fillRect(p.x, p.y, p.w, p.h);
      if (p.type === "crumble") {
        ctx2.fillStyle = "#000";
        ctx2.beginPath();
        ctx2.moveTo(p.x + 5, p.y);
        ctx2.lineTo(p.x + 15, p.y + p.h);
        ctx2.stroke();
      }
      ctx2.fillStyle = "rgba(255,255,255,0.3)";
      ctx2.fillRect(p.x, p.y, p.w, 4);
      ctx2.strokeStyle = p.border || currentBiome.border;
      ctx2.lineWidth = 2;
    ...
    itemsRef.current.forEach((item) => {
      if (item.type === "balloon") {
        ...
        ctx2.lineWidth = 1;
        ctx2.beginPath();
        ctx2.moveTo(item.x + 15, item.y + 40);
        ctx2.lineTo(item.x + 15 + Math.sin(performance.now() * 0.01) * 5, item.y + 80);
        ctx2.stroke();
        ctx2.fillStyle = "red";
        ctx2.beginPath();
        ctx2.ellipse(item.x + 15, item.y + 20, 15, 20, 0, 0, Math.PI * 2);
        ctx2.fill();
        ctx2.fillStyle = "white";
        ctx2.beginPath();
        ctx2.arc(item.x + 10, item.y + 10, 4, 0, Math.PI * 2);
        ctx2.fill();
      }
    });
    const tilt = (inputRef.current - player2.x) * 0.05;
    drawRig(ctx2, player2.x, player2.y, player2.vy, tilt);
    ctx2.fillStyle = currentBiome.text;
    ctx2.font = "bold 20px monospace";
    ctx2.textAlign = "left";
    ctx2.fillText(`${currentBiome.name}`, 10, 30);
    ctx2.fillText(`${Math.floor(scoreRef.current)}m`, 10, 50);
    if (currentBiome.glitch && Math.random() > 0.95) {
      ctx2.restore();
    }
    requestRef.current = requestAnimationFrame(gameLoop);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "page-enter", style: {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", flexDirection: "column", alignItems: "center" }, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { position: "relative", width: "400px", height: "600px" }, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: {
      ...
      STREAKS.map((streak) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { onClick: () => buyStreak(streak), style: {
          ...
          streak.gradient && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
            height: "10px",
            borderRadius: "5px",
            marginBottom: "10px",
