import { s as supabase } from "./index-1m1nRMjW.js";
const MOCK_DATA = [];
const LeaderboardService = {
  getTopScores: async (gameId) => {
    try {
      if (!supabase) return MOCK_DATA;
      const { data, error } = await supabase.from("profiles").select("display_name, high_scores, coins, friend_code, xp, squad").order("xp", { ascending: false }).limit(200);
      const STAT_MAP = {
