import { r as reactExports, a as useGamification, b as useRetroSound, j as jsxRuntimeExports, S as SquishyButton, L as Link, t as triggerConfetti } from "./index-BzABmepQ.js";
...
const FlappyMascot = () => {
  ...
  const ASSETS = {
    bird: "/assets/merchboy_face.png",
    bunny: "/assets/merchboy_bunny.png",
    coin: "/assets/merchboy_money.png"
  };
  reactExports.useEffect(() => {
    gameState.current.images = {};
    const birdSrc = shopState?.equipped?.flappy === "flappy_bunny" ? ASSETS.bunny : ASSETS.bird;
    const birdImg = new Image();
    birdImg.src = birdSrc;
    gameState.current.images.bird = birdImg;
    const coinImg = new Image();
    coinImg.src = ASSETS.coin;
    gameState.current.images.coin = coinImg;
  ...
  const initGame = () => {
    ...
    gameState.current = {
      ...
      animationId: null,
      frame: 0,
      images: gameState.current.images
  ...
  const gameLoop = () => {
    ...
    if (lastPipe && GAME_WIDTH - lastPipe.x >= PIPE_SPACING) {
      ...
      if (Math.random() > 0.5) {
        state.coins.push({ x: GAME_WIDTH + 50, y: h + 75 + (Math.random() * 40 - 20), collected: false });
      }
    }
    ...
    state.pipes.forEach((p) => {
      if (50 + BIRD_SIZE - 5 > p.x && 50 + 5 < p.x + 60) {
        if (state.birdY + 5 < p.topHeight || state.birdY + BIRD_SIZE - 5 > p.topHeight + 150) endGame();
      }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const y = 100 + Math.sin(i * 1321) * 20;
      ctx.beginPath();
    ...
    state.coins.forEach((c) => {
      ...
      ctx.rotate(Math.sin(state.frame * 0.1) * 0.5);
      const img = gameState.current.images.coin;
      if (img && img.complete) {
    ...
    const birdImg = gameState.current.images.bird;
