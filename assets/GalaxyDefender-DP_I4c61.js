import { a as useGamification, r as reactExports, b as useRetroSound, j as jsxRuntimeExports, S as SquishyButton, L as Link, t as triggerConfetti } from "./index-oyUaBna8.js";
...
const GalaxyDefender = () => {
  const { updateStat, incrementStat, shopState, addCoins, userProfile, stats, addZonePoints } = useGamification() || { updateStat: () => {
  }, incrementStat: () => {
  }, shopState: null };
  ...
  const ASSETS = {
    ship: "/assets/merchboy_face.png",
    enemy1: "/assets/merchboy_bunny.png",
    enemy2: "/assets/merchboy_cat.png",
    boss: "/assets/merchboy_money.png"
  };
  reactExports.useEffect(() => {
    ...
    const loadImg = (key, src) => {
      const img = new Image();
      img.src = src;
      gameState.current.images[key] = img;
    };
    loadImg("ship", ASSETS.ship);
    loadImg("enemy1", ASSETS.enemy1);
    loadImg("enemy2", ASSETS.enemy2);
    loadImg("boss", ASSETS.boss);
  ...
  const endGame = (win) => {
    ...
    if (addZonePoints && finalScore > 200) {
      const points = Math.floor(finalScore / 20);
  ...
  const gameLoop = (timestamp) => {
    ...
    if (gameActiveRef.current) {
      ...
      if (state.boss) {
        ...
        if (Math.random() < 5e-3 && state.powerups.length === 0) {
    ...
    if (state.invincible % 10 < 5) {
      ...
      if (shipImg && shipImg.complete) {
        ...
        ctx.drawImage(shipImg, -PLAYER_SIZE / 2, -PLAYER_SIZE / 2, PLAYER_SIZE, PLAYER_SIZE);
      } else {
        ctx.fillStyle = "cyan";
        ctx.beginPath();
        ctx.moveTo(playerXDraw + PLAYER_SIZE / 2, playerYDraw);
        ctx.lineTo(playerXDraw + PLAYER_SIZE, playerYDraw + PLAYER_SIZE);
        ctx.lineTo(playerXDraw, playerYDraw + PLAYER_SIZE);
        ctx.fill();
      }
      if (state.hasShield) {
    ...
    state.enemies.forEach((e) => {
      const type = (e.x + e.y) % 20 > 10 ? "enemy1" : "enemy2";
      const img = images?.[type];
      if (img && img.complete) {
        ctx.drawImage(img, e.x, e.y, ENEMY_SIZE, ENEMY_SIZE);
      } else {
    ...
    if (state.boss) {
      ...
      if (bossImg && bossImg.complete) {
        ctx.drawImage(bossImg, state.boss.x, state.boss.y, BOSS_SIZE, BOSS_SIZE);
      } else {
        ctx.fillStyle = "red";
