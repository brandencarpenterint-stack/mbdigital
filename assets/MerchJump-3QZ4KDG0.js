import { r as reactExports, b as useRetroSound, a as useGamification, j as jsxRuntimeExports, S as SquishyButton } from "./index-oyUaBna8.js";
import { G as GameOverCard } from "./GameOverCard-CmwYLZY1.js";
const WIDTH = 400;
const HEIGHT = 600;
const GRAVITY = 0.4;
const JUMP_FORCE = -13;
const MerchJump = () => {
  ...
  const ASSETS = {
    player: "/assets/merchboy_face.png",
    item: "/assets/merchboy_money.png"
  };
  reactExports.useEffect(() => {
    stateRef.current.images = {};
    const pImg = new Image();
    pImg.src = ASSETS.player;
    stateRef.current.images.player = pImg;
    const iImg = new Image();
    iImg.src = ASSETS.item;
    stateRef.current.images.item = iImg;
  ...
  const gameLoop = () => {
    ...
    state.items.forEach((i) => {
      ...
      const itemImg = images?.item;
      if (itemImg && itemImg.complete) {
        ctx.drawImage(itemImg, i.x - 15, i.y - 15, 30, 30);
      } else {
        ctx.fillStyle = "red";
        ctx.beginPath();
    ...
    if (playerImg && playerImg.complete) {
      ...
      ctx.drawImage(playerImg, -25, -30, 50, 60);
      ctx.restore();
    } else {
      ctx.fillStyle = "white";
