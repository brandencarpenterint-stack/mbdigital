import { a as useGamification, b as useRetroSound, r as reactExports, j as jsxRuntimeExports, S as SquishyButton, t as triggerConfetti } from "./index-BzABmepQ.js";
const GRID_ROWS = 4;
const GRID_COLS = 4;
const CELL_SIZE = 100;
const GAP = 10;
...
const ASSETS = {
  normal: "/assets/merchboy_face.png",
  cyber: "/assets/merchboy_bunny.png",
  // Or another variant if available
  gold: "/assets/merchboy_money.png"
};
const MOLE_TYPES = {
  normal: { score: 10, hp: 1 },
  cyber: { score: 50, hp: 2 },
  gold: { score: 100, hp: 1, speed: 2 }
};
  ...
  const draw = () => {
    ...
    state.moles.forEach((m) => {
      ...
      if (img && img.complete) {
        ...
        let shakeX = 0;
        if (m.state === "HIT") shakeX = (Math.random() - 0.5) * 10;
        ctx.drawImage(img, x + shakeX, drawY, CELL_SIZE, CELL_SIZE);
        if (m.state === "HIT") {
          ctx.fillStyle = "rgba(255, 0, 0, 0.5)";
          ctx.fillRect(x, drawY, CELL_SIZE, CELL_SIZE);
        }
      } else {
