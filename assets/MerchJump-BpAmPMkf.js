import { r as reactExports, c as useRetroSound, b as useGamification, j as jsxRuntimeExports, S as SquishyButton, L as Link } from "./index-DeepPATn.js";
import { G as GameOverCard } from "./GameOverCard-D3KBZSxg.js";
import { feedService } from "./feed-BmMQ93_c.js";
const BIOMES = [
...
const MerchJump = () => {
  ...
  const generatePlatform = (y, biome) => {
    ...
    if (Math.abs(score2 - 5e3) < 500 && Math.random() < 0.1 && itemsRef.current.length === 0) {
    ...
    if (Math.abs(score2 - 2500) < 400 && Math.random() < 0.05 && itemsRef.current.length === 0) {
      itemsRef.current.push({
  ...
  const gameLoop = () => {
    ...
    itemsRef.current.forEach((item) => {
      if (item.type === "balloon") {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(item.x + 15, item.y + 40);
        ...
        ctx.fillStyle = "white";
        ctx.beginPath();
        ctx.arc(item.x + 10, item.y + 10, 4, 0, Math.PI * 2);
        ctx.fill();
      } else if (item.type === "merch_balloon") {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "page-enter", style: {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", flexDirection: "column", alignItems: "center" }, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { position: "relative", width: "400px", height: "600px" }, children: [
        ...
        gameState !== "PLAYING" && /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: gameState === "GAMEOVER" ? /* @__PURE__ */ jsxRuntimeExports.jsx(GameOverCard, { score: Math.floor(scoreRef.current), bestScore: highScore, gameId: "merch_jump", onReplay: initGame, onHome: () => window.location.href = "/arcade" }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { position: "absolute", inset: 0, background: "rgba(255,255,255,0.9)", backdropFilter: "blur(5px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#333", borderRadius: "10px" }, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { width: "250px", height: "600px", background: "#1a1a1a", borderRadius: "20px", border: "2px solid #444", padding: "20px", display: "flex", flexDirection: "column", color: "white", overflowY: "auto" }, children: [
